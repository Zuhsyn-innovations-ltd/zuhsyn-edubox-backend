// screens/BiologySubjectScreen.js
  ...
  useEffect(() => {
    const loadTopics = async () => {
      try {
        ...
        const results = await db.executeSql(`SELECT * FROM biology_topics ORDER BY id ASC`);
        ...
        if (results[0].rows.length > 0) {
          ...
          for (const topic of defaultTopics) {
                      await db.executeSql(`INSERT INTO biology_topics (title) VALUES (?);`, [topic]);
                    }
              ...
              <ScrollView style={styles.container}>
                ...
                {topics.map((topic, index) => (
                  <TouchableOpacity
                    ...
                    style={styles.topic}
                    onPress={() =>
                      navigation.navigate('BiologyLessonDetail', { topicIndex: index })
                    }
                  >
          ...
          export default BiologycSubjectScreen;
